...
class ETableActionButtons extends StatelessWidget {
  const ETableActionButtons({
    super.key,
    this.view = false,
    this.edit = true,
    this.delete = true,
      ...
      children: [
        if (view)
          IconButton(
            ...
            icon: const Icon(Iconsax.eye, color: EColors.darkerGrey),
          ),
        if (edit)
          IconButton(
            onPressed: onEditPressed,
            icon: const Icon(Iconsax.pen_add, color: EColors.primaryColor),
          ),
        if (delete)
          IconButton(
            onPressed: onDeletePressed,
            icon: const Icon(Iconsax.trash, color: EColors.error),
          ),
      ],
    );
  }
